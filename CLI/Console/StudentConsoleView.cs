...
class StudentConsoleView
{
    ...
    private Student InputStudent()
    {
        ...
        System.Console.WriteLine("Unesite datum rodjenja studenta (format: dd.MM.yyyy): ");
        string datum_rodjenja_input = System.Console.ReadLine() ?? string.Empty;

        if (DateTime.TryParseExact(datum_rodjenja_input, "dd.MM.yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime datum_rodjenja))
        {
            // Successfully parsed the input string to DateTime
            // Now you can use 'datum_rodjenja' in your Student constructor
        }
        else
        {
            // Failed to parse the input string to DateTime
            System.Console.WriteLine("Neispravan format datuma. Unesite datum u formatu dd/MM/yyyy.");
        ...
        string kontakt_telefon = System.Console.ReadLine() ?? string.Empty;
        ...
        double prosecna_ocena = ConsoleViewUtils.SafeInputDouble();
        ...
        List<Ocena> polozeni_ispiti = inputp
    .Split(' ')
    .Select(p => new Ocena())
    .ToList();
        ...
        List<Predmet> nepolozeni_ispiti = inputnp
    .Split(' ')
    .Select(p => new Predmet())
    .ToList();
